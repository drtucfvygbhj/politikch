...
## To review
- **Vote arguments reproduction** (`data/overviews/initiative/*.json`, from `scripts/fetch_arguments.py`): the for/against arguments are reproduced **verbatim** from the Federal Council's official voting explanations ("Erläuterungen des Bundesrates") on the Art. 5 URG official-texts basis (see `NOTICE.md`). The *against* text and the neutral parts are plainly official; the *for* text is authored by the initiative/referendum committee and printed inside the official brochure. Worth a **native/legal confirmation** that reproducing the committee-authored *for* arguments verbatim (labelled as the committee's words, shown alongside the official *against* arguments, attributed and linked) is fine on that basis. Until confirmed, this is believed compliant and non-partisan, but flag before relying on it publicly at scale.
...
- **Voting / community poll / My-profile + poll privacy** (`mine.*`, `profile.*`, `privacy.h.poll`, `privacy.p.poll`, `privacy.p.poll2`, and the reworded `privacy.lead` / `privacy.p.storage`): EN/DE/FR/IT confident; **Romansh (rm)** best-effort — native review recommended, especially the privacy poll-disclosure wording.
